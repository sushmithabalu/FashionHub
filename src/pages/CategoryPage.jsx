import React from "react";
import { useNavigate, useParams } from "react-router-dom";
import Layout from "../components/Layout";
import { categories } from "../data/products";

const subImages = {
  Shirts: "https://images.unsplash.com/photo-1596755094514-f87e34085b2c?w=400&q=80",
  Pants: "https://images.unsplash.com/photo-1473966968600-fa801b869a1a?w=400&q=80",
  Jeans: "https://images.unsplash.com/photo-1555689502-c4b22d76c56f?w=400&q=80",
  Shorts: "https://encrypted-tbn3.gstatic.com/shopping?q=tbn:ANd9GcQbwG7obaE5cxhSsCZ2D3ryepAjPFJza1SgDk_NfYLpcMnEWGPkLPK4A8qu0qy6RPBCgHoQ5DwmK6gvnX5QUIkYeFdRh0wx322sEpYFLyUE6BWP0fgnqtpqe0f5j0eTmYqXmDj6ibqQsR7u&usqp=CAc",
  Saree: "https://images.unsplash.com/photo-1610030469983-98e550d6193c?w=400&q=80",
  Kurti: "data:image/webp;base64,",
  Tops: "https://images.unsplash.com/photo-1509631179647-0177331693ae?w=400&q=80",
  Skirts: "https://images.unsplash.com/photo-1583496661160-fb5886a0aaaa?w=400&q=80",
  "Crop Tops": "https://images.unsplash.com/photo-1618932260643-eee4a2f652a6?w=400&q=80",
  "Boys Wear": "https://encrypted-tbn3.gstatic.com/shopping?q=tbn:ANd9GcRNru3ZGf9G0WbgqdMJ7iQb0jJ4_wtfCKa-nNy5jCY5LN53B0DwcBJ_jX1tfrGt8J5Sz6cdCVfBEV9d2ty4dq_MjLjIrfGFhJjAqEiDxtZo0gmoasp2m_oalw",
  "Girls Wear": "https://encrypted-tbn1.gstatic.com/shopping?q=tbn:ANd9GcQra_Eog5BFANN-r4xXNeMuxknAvGo4X43DXaag7X3C-FZZbJCyUhFZFlWvko2tDuLr-SdZiiIFA3JrWrcqzRY0xSIKsAdL8ZZFFVJE9ipZ",
};

export default function CategoryPage() {
  const { category } = useParams();
  const navigate = useNavigate();
  const cat = categories[category];
  if (!cat) return <Layout><div style={{ padding: 40, textAlign: "center" }}>Category not found</div></Layout>;

  return (
    <Layout>
      <div style={{ padding: "28px 24px" }}>
        <div style={{ fontSize: 12, color: "#0066c0", marginBottom: 8 }}>
          <span onClick={() => navigate("/")} style={{ cursor: "pointer" }}>Home</span> &rsaquo; {cat.label}
        </div>
        <h1 style={{ fontSize: 26, fontWeight: 800, color: "#131921", marginBottom: 4 }}>
          {cat.label}'s Fashion
        </h1>
        <p style={{ color: "#666", fontSize: 14, marginBottom: 28 }}>
          Browse {cat.subcategories.length} categories · Thousands of products
        </p>
        <div style={{
          display: "grid",
          gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))",
          gap: 20,
        }}>
          {cat.subcategories.map((sub) => (
            <div
              key={sub}
              onClick={() => navigate(`/products/${encodeURIComponent(sub)}`)}
              style={{
                background: "#fff", borderRadius: 16,
                overflow: "hidden", cursor: "pointer",
                boxShadow: "0 2px 12px rgba(0,0,0,0.08)",
                transition: "transform 0.2s, box-shadow 0.2s",
              }}
              onMouseEnter={(e) => { e.currentTarget.style.transform = "translateY(-4px)"; e.currentTarget.style.boxShadow = "0 12px 30px rgba(0,0,0,0.15)"; }}
              onMouseLeave={(e) => { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = "0 2px 12px rgba(0,0,0,0.08)"; }}
            >
              <div style={{ position: "relative", paddingTop: "80%", overflow: "hidden" }}>
                <img
                  src={subImages[sub] || "https://images.unsplash.com/photo-1489987707025-afc232f7ea0f?w=400&q=80"}
                  alt={sub}
                  style={{ position: "absolute", top: 0, left: 0, width: "100%", height: "100%", objectFit: "cover" }}
                />
                <div style={{
                  position: "absolute", bottom: 0, left: 0, right: 0,
                  background: "linear-gradient(to top, rgba(0,0,0,0.7), transparent)",
                  padding: "20px 14px 12px",
                }}>
                  <div style={{ color: "#fff", fontWeight: 800, fontSize: 16 }}>{sub}</div>
                </div>
              </div>
              <div style={{ padding: "12px 14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontSize: 12, color: "#666" }}>Explore collection</span>
                <span style={{ color: "#f90", fontWeight: 700, fontSize: 14 }}>→</span>
              </div>
            </div>
          ))}
        </div>
      </div>
    </Layout>
  );
}